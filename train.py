...
from prepare import DEFAULT_DEVICE, MAX_SEQ_LEN, TIME_BUDGET, Tokenizer, make_dataloader, evaluate_bpb
...
ENABLE_COMPILE = env_bool("AUTORESEARCH_ENABLE_COMPILE", USE_CUDA)
...
def maybe_compile_module(module, **compile_kwargs):
    if ENABLE_COMPILE:
...
def get_peak_memory_mb():
    ...
    if USE_MPS and hasattr(torch.mps, "driver_allocated_memory"):
        return torch.mps.driver_allocated_memory() / 1024 / 1024
    return 0.0


FLASH_ATTN = None
ATTN_BACKEND = "scaled_dot_product_attention"
if USE_CUDA and get_kernel is not None:
    try:
        cap = torch.cuda.get_device_capability()
        repo = "varunneal/flash-attention-3" if cap == (9, 0) else "kernels-community/flash-attn3"
        FLASH_ATTN = get_kernel(repo).flash_attn_interface
        ATTN_BACKEND = f"flash_attn ({repo})"
    except Exception as exc:
        ATTN_BACKEND = f"scaled_dot_product_attention (flash-attn unavailable: {exc.__class__.__name__})"
...
def causal_attention(q, k, v, window_size):
    ...
    if 0 < window < q.size(-2):
        positions = torch.arange(q.size(-2), device=q.device)
        distance = positions[:, None] - positions[None, :]
        allowed = (distance >= 0) & (distance < window)
        attn_mask = torch.full((q.size(-2), q.size(-2)), float("-inf"), device=q.device, dtype=q.dtype)
        attn_mask.masked_fill_(allowed, 0.0)
        is_causal = False
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def __init__(self, param_groups):
        ...
        # Keep scalars on CPU only for the compiled CUDA path.
        scalar_device = "cpu" if ENABLE_COMPILE else DEVICE
    ...
    def _step_muon(self, group):
        ...
        updated_params = list(stacked_params.unbind(0))
        if USE_CUDA:
            torch._foreach_copy_(params, updated_params)
        else:
            for param, updated in zip(params, updated_params):
                param.copy_(updated)
...
WINDOW_PATTERN = env_str("AUTORESEARCH_WINDOW_PATTERN", "SSSL" if USE_CUDA else "L")

# Optimization
TOTAL_BATCH_SIZE = env_int("AUTORESEARCH_TOTAL_BATCH_SIZE", 2**19 if USE_CUDA else 2**14)
...
# Model size
DEPTH = env_int("AUTORESEARCH_DEPTH", 8 if USE_CUDA else 4)               # number of transformer layers
DEVICE_BATCH_SIZE = env_int("AUTORESEARCH_DEVICE_BATCH_SIZE", 128 if USE_CUDA else 4)  # per-device batch size (reduce if OOM)
...
device = DEVICE
autocast_ctx = torch.amp.autocast(device_type="cuda", dtype=COMPUTE_DTYPE) if USE_CUDA else nullcontext()
DEVICE_PEAK_FLOPS = 989.5e12 if USE_CUDA else None

tokenizer = Tokenizer.from_directory()
vocab_size = tokenizer.get_vocab_size()
print(f"Vocab size: {vocab_size:,}")
print(f"Device: {device} | attention: {ATTN_BACKEND} | compile: {ENABLE_COMPILE}")
...
model = maybe_compile_module(model, dynamic=False)
...
while True:
    ...
    mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE / dt / DEVICE_PEAK_FLOPS if DEVICE_PEAK_FLOPS else None
...
steady_state_mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE * (step - 10) / total_training_time / DEVICE_PEAK_FLOPS if total_training_time > 0 and DEVICE_PEAK_FLOPS else None

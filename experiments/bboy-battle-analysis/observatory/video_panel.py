...
from observatory.body_state import BONE_PAIRS, BodyState
...
def render_video_panel(
    ...
    span = max(positions[:, 0].max() - positions[:, 0].min(),
               positions[:, 2].max() - positions[:, 2].min(), 0.5)

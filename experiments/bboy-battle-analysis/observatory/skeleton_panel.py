...
from observatory.body_state import BONE_PAIRS, BodyState, TemporalWindow
from observatory.color_system import (
    BG_COLOR,
    BONE_WIDTH,
    COM_COLOR,
    COM_DOT_RADIUS,
...
def _project_3d_to_2d(
    ...
    Simple orthographic projection: X → screen X, Z → screen Y (inverted).
    Y axis (depth) is ignored for the main view.
    """
    x = (point[0] - center_offset[0]) * scale + panel_w / 2
    y = panel_h - ((point[2] - center_offset[2]) * scale + panel_h * 0.15)
    return int(x), int(y)
...
def _compute_projection_params(
    ...
    # Compute bounding box of current skeleton
    span_x = positions[:, 0].max() - positions[:, 0].min()
    span_z = positions[:, 2].max() - positions[:, 2].min()
    max_span = max(span_x, span_z, 0.5)  # at least 0.5m

...
def compute_audio_signal(video_path: str, n_frames: int, fps: int = 30, sigma_ms: float = 50.0) -> np.ndarray:
    ...
    if not HAS_LIBROSA:
        ...
        # Synthetic 120 BPM beat signal (2 beats per second)
        bpm = 120
        ...
        for b in beat_times:
            ...
            if 0 <= frame_idx < n_frames:
                H += np.exp(-0.5 * ((np.arange(n_frames) - frame_idx) / sigma_frames) ** 2)
...
def main():
    ...
    if args.no_audio or args.video is None:
        H, beat_times = compute_audio_signal("", len(M), fps=args.fps)

...
if [ "${GPU_MEM}" -lt 15000 ]; then
    echo "  WARNING: ${GPU_MEM}MB VRAM is below recommended 16GB. May OOM."
fi
...
pip install -q "git+https://github.com/facebookresearch/pytorch3d.git" 2>/dev/null || {
    echo "  pytorch3d from git failed. Trying conda..."
    conda install -y -c pytorch3d pytorch3d 2>/dev/null || {
        echo "  WARNING: pytorch3d install failed. GVHMR may not work."
        echo "  Try: pip install pytorch3d -f https://dl.fbaipublicfiles.com/pytorch3d/packaging/wheels/py310_cu121_pyt230/download.html"
    }
}
...
try:
    # Try the GVHMR download script if it exists
...
" 2>/dev/null || echo "  Auto-download not available. Manual download needed."
else
